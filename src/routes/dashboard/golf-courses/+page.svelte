			...
			<svelte:fragment slot="cell-address" let:item>
				{#if item.address && typeof item.address === 'object'}
					...
					<div class="group relative">
						<span
							class="block cursor-help truncate text-gray-900 dark:text-gray-200"
							title={fullAddress}
						>
							{fullAddress || '-'}
						</span>
						{#if fullAddress && fullAddress.length > 25}
							<div
								class="pointer-events-none absolute bottom-full left-0 z-[100] mb-2 hidden max-w-xs min-w-[200px] rounded-md bg-gray-900 px-3 py-2 text-xs text-white shadow-lg group-hover:block dark:bg-gray-700"
							>
								{fullAddress}
								<div
									class="absolute -bottom-1 left-4 h-2 w-2 rotate-45 bg-gray-900 dark:bg-gray-700"
								></div>
							</div>
						{/if}
					</div>
