<script lang="ts">
	...
	$: stats = storeState
		? ([
				...
				{
					label: '운행 가능',
					value: storeState.items.filter((c: Cart) => c.cartStatus.currentState === 'available')
						.length,
					icon: CheckCircle,
				...
				{
					label: '정비 중',
					value: storeState.items.filter((c: Cart) => c.cartStatus.currentState === 'maintenance')
						.length,
					icon: AlertTriangle,
				...
				{
					label: '고장',
					value: storeState.items.filter((c: Cart) => c.cartStatus.currentState === 'broken')
						.length,
					icon: XCircle,
					color: 'text-red-500'
				}
