<script lang="ts">
	...
	import {
		cartInventoryStore,
		cartInventoryStoreInstance,
		isLoading,
		errorMessage,
		selectedCount
	} from '$lib/stores/cart-inventory.store';
	...
	$: stats = storeState
		? ([
				...
				{
					label: '창고 보관',
					value: storeState.items.filter((c: Cart) => c.status === 'in_warehouse').length,
					icon: Package,
					color: 'text-green-500'
				},
				{
					label: '현장 배치',
					value: storeState.items.filter((c: Cart) => c.status === 'deployed').length,
					icon: MapPin,
					color: 'text-purple-500'
				},
				{
					label: '정비 중',
					value: storeState.items.filter((c: Cart) => c.status === 'maintenance').length,
					icon: Wrench,
					color: 'text-red-500'
				}
			] as StatItem[])
	...
	async function handleDeploymentSave(event: CustomEvent) {
		...
		if (cartToDeploy) {
			const success = await cartInventoryStoreInstance.deployCart(
				cartToDeploy.id,
				golfCourseId,
				golfCourseName
			);
...
{#if error}
	<div
		class="mb-4 rounded-lg border border-red-500/50 bg-red-50 p-4 dark:bg-red-900/50"
		role="alert"
	>
			...
			>
				<svelte:fragment slot="filters">
					<select
						...
						on:change={(e) =>
							cartInventoryStoreInstance.changeStatusFilter(
								e.currentTarget.value as 'all' | 'in_warehouse' | 'deployed' | 'maintenance'
							)}
