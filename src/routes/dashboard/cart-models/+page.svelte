<script lang="ts">
	...
	import { cartModelStore, cartModelStoreInstance, isLoading, errorMessage, selectedCount } from '$lib/stores/cart-model.store';
	...
	$: stats = storeState ? ([
		...
		{ label: '활성 모델', value: storeState.items.filter((m: CartModel) => m.status === 'active').length, icon: Activity, color: 'text-green-500' },
		{ label: '단종 모델', value: storeState.items.filter((m: CartModel) => m.status === 'discontinued').length, icon: AlertCircle, color: 'text-red-500' },
		{ label: '2024년 모델', value: storeState.items.filter((m: CartModel) => m.year === 2024).length, icon: Settings, color: 'text-purple-500' }
	] as StatItem[]) : [];
	...
	function getStatusInfo(status: string) {
		switch (status) {
			...
			case 'discontinued': return { color: 'text-red-600 bg-red-100 dark:text-red-400 dark:bg-red-900/50', text: '단종' };
...
<div class="p-4 md:p-6">
	...
	{#if error}
		<div class="mb-4 flex items-center justify-between rounded-lg bg-red-50 p-4 dark:bg-red-900/20">
			...
			<button on:click={() => errorMessage.set(null)} class="text-red-500 hover:text-red-700" aria-label="Close error message">
				<X class="h-4 w-4" />
			</button>
		</div>
	{/if}
	...
	>
		<svelte:fragment slot="filters">
			<select
				...
				on:change={(e) => cartModelStoreInstance.changeFilter(e.currentTarget.value as 'all' | 'active' | 'discontinued')}
