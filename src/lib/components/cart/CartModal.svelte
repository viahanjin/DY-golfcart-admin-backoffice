		...
		<div class="space-y-6 p-6">
			...
			<div class="space-y-4 rounded-lg border p-4 dark:border-gray-700">
				...
				<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
					<div><label for="cartId" class="dark:text-gray-300">카트 ID *</label><input id="cartId" type="text" bind:value={formData.id} disabled={isReadOnly} class="w-full rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white" /></div>
					<div><label for="cartName" class="dark:text-gray-300">카트명/별명</label><input id="cartName" type="text" bind:value={formData.cartName} disabled={isReadOnly} class="w-full rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white" /></div>
					<div><label for="manufacturer" class="dark:text-gray-300">제조사</label><input id="manufacturer" type="text" bind:value={formData.manufacturer} disabled={isReadOnly} class="w-full rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white" /></div>
					<div><label for="modelYear" class="dark:text-gray-300">제조년도</label><input id="modelYear" type="text" bind:value={formData.modelYear} disabled={isReadOnly} class="w-full rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white" /></div>
					<div><label for="assignedGolfCourseId" class="dark:text-gray-300">할당 골프장 *</label>
				...
				<div class="space-y-4 rounded-lg border p-4 dark:border-gray-700">
					...
					{#each sensorOptions as sensor}
						<div class="flex items-center gap-2">
							...
							<label for="sensor-{sensor.id}" class="dark:text-gray-300">{sensor.label}</label>
						</div>
					{/each}
				</div>
			</div>
